...
const STATUS_CONFIG: Record<Status, { label: string; badge: string; dot: string; border: string; ring: string }> = {
  todo:        { label: "To Do",       badge: "text-slate-400 bg-slate-400/10 border-slate-400/25",  dot: "bg-slate-400",  border: "border-slate-600",  ring: "border-slate-400" },
  in_progress: { label: "In Progress", badge: "text-amber-400 bg-amber-400/10 border-amber-400/25",  dot: "bg-amber-400",  border: "border-slate-600",  ring: "border-amber-400" },
    ...
    >
      <div className="bg-slate-800 rounded-2xl border border-slate-700 p-8 w-full max-w-md shadow-2xl">
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="px-4 py-2.5 rounded-lg bg-slate-900 border border-slate-700 text-slate-400 text-sm font-medium cursor-pointer hover:border-slate-500 transition-colors">
            Cancel
          </button>
          <button
  ...
  return (
    ...
    >
      <div className="bg-slate-800 rounded-2xl border border-slate-700 p-8 w-full max-w-sm shadow-2xl">
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 px-4 py-2.5 rounded-lg bg-slate-900 border border-slate-700 text-slate-400 text-sm font-medium cursor-pointer hover:border-slate-500 transition-colors">
            Cancel
          </button>
          <button
...
function CreateTaskModal({ projectId, onClose, onCreated }: {
  ...
  const [form, setForm] = useState<Omit<CreateTaskInput, "projectId">>({ title: "", description: "", status: "todo" });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {(["all", "todo", "in_progress", "done"] as const).map(f => (
            ...
            >
              ...
              <span className={`ml-2 text-xs px-1.5 py-0.5 rounded-full ${filter === f ? "bg-indigo-500/25 text-indigo-300" : "bg-slate-800 text-slate-500"}`}>
                {counts[f]}
              </span>
            </button>
          ))}
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-slate-500 flex flex-col items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-indigo-500/10 border border-dashed border-indigo-500/40 flex items-center justify-center text-3xl">📋</div>
            <p>No tasks here.</p>
            <button onClick={() => setShowModal(true)} className="px-5 py-2.5 rounded-lg bg-indigo-500 text-white text-sm font-semibold hover:bg-indigo-400 transition-colors cursor-pointer">
